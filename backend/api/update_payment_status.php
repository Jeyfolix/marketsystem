...
// CRITICAL FIX: Force to integer and remove any quotes
...
try {
    // CRITICAL: Use bindValue with PDO::PARAM_INT for the ID
    $query = "UPDATE transactions SET status = :status, verified_by = :verified_by, verified_at = NOW() WHERE id = :id";
    
    $stmt = $db->prepare($query);
    
    // Bind with explicit types
    $stmt->bindValue(':status', $status, PDO::PARAM_STR);
    $stmt->bindValue(':verified_by', $verified_by, PDO::PARAM_STR);
    $stmt->bindValue(':id', $payment_id, PDO::PARAM_INT); // This is the key!
    ...
    if ($stmt->execute()) {
        if ($stmt->rowCount() > 0) {
            ...
            $check = $db->prepare("SELECT id FROM transactions WHERE id = :id");
            $check->bindValue(':id', $payment_id, PDO::PARAM_INT);
            $check->execute();
